...
function normalizeHeadingLine(line: string) {
  ...
  const patterns = [
    new RegExp(`^${chineseNumber}[、.．]\\s*(.+)$`),
    new RegExp(`^第${chineseNumber}部分[：:]?\\s*(.+)$`),
    new RegExp(`^第${chineseNumber}步[：:]?\\s*(.+)$`),
    /^(\d+)[、.．]\s*(.+)$/,
  ];
  ...
  const label = trimmed.match(/^(问题背景|问题现象|平台信息|相关日志|相关代码路径|排查过程|修改方案|Diff 记录|验证方法|最终结论|快速判断|解决方案|说明|原因分析|验证结果|操作步骤)[：:]?$/);
...
function inferFenceLanguage(code: string) {
  ...
  if (/(adb\s+|fastboot\s+|source\s+build\/envsetup|lunch\s+|make\s+|\.\/|^cd\s+)/m.test(trimmed)) return 'bash';
  if (/(avc: denied|E\/|W\/|I\/|D\/|FATAL|Exception|backtrace|logcat|bootreason)/i.test(trimmed)) return 'log';
...
function normalizeBody(file: string, body: string, title: string) {
...
const changed = walk(docsRoot).filter(normalizeFile);
console.log(`normalize-notes: checked ${walk(docsRoot).length} files, changed ${changed.length} files.`);
